...
import com.alibaba.easyexcel.test.util.DataUtil;
...
/**
 * @see 'https://github.com/weiwosuoai/spring-boot-tutorial/tree/master/spring-boot-excel'
 */
public class WriteTest {

    private static final String FILE_PATH = "C:\\cmbox-service\\";
    ...
    public void writeExcel1() throws Exception {
        // 文件输出位置
        OutputStream out = new FileOutputStream(FILE_PATH + "test.xlsx");
    ...
    public void writeExcel2() throws Exception {
        // 文件输出位置
        OutputStream out = new FileOutputStream(FILE_PATH + "test2.xlsx");
...
//        table1.setTableStyle(DataUtil.createTableStyle());
    ...
    public void writeChongqingExcel() throws Exception {
        OutputStream out = new FileOutputStream(FILE_PATH + "chongqing.xlsx");
        ExcelWriter writer = EasyExcelFactory.getWriter(out);
...
//        // 无注解的模式，动态添加表头
    ...
    private List<WriteModel2> createModel2List() {
        List<WriteModel2> writeModels2 = new ArrayList<>();

        for (int i = 0; i < 100; i++) {
            writeModels2.add(new WriteModel2(String.valueOf(i), "犬小哈", LocalDateTime.now()));
        }
